...
from torchFewShot.models.protohead import few_shot_class
...
def main():
    ...
    classifier = models.init_model(name=args.cls_arch, scale_cls=args.scale_cls, num_classes=args.num_classes)
    print("Model size: {:.3f} M".format(count_num_param(model)))

    criterion = CrossEntropyLoss(num_classes=args.num_classes, epsilon=args.epsilon)
    ...
    for epoch in range(args.start_epoch, args.max_epoch):
        ...
        train(epoch, model, classifier, criterion, optimizer, trainloader, learning_rate, use_gpu)
        train_time += round(time.time() - start_train_time)
        
        if epoch > (args.stepsize[0]-1) or (epoch + 1) % args.eval_step == 0 or (epoch + 1) == args.max_epoch:
            print("==> Test 5-way-1-shot")
            acc_1shot = test(model, testloader)
...
def train(epoch, model, classifier, criterion, optimizer, trainloader, learning_rate, use_gpu):
    losses = AverageMeter()
    accs = AverageMeter()
    ...
    for batch_idx, (imgs, pids) in enumerate(trainloader):
        ...
        acc = (torch.sum(preds == pids.detach().cpu()).float()) / pids.size(0)
